...
/*void print_char_5x8(uint8_t column, uint8_t string, uint8_t sign) // 0..63 / 0..5 / Символ
{	
...
/*unsigned char print_arial(uint8_t column, uint8_t page, uint8_t sign) // 0..63 / 0..5 / Символ
{	
	sign += 68; //Приплюсовываем чтобы попасть в нужное значение массива
	unsigned char fontHeight = pgm_read_byte(&arial_narrow[1]); // Высота
	unsigned char amountOfElems = pgm_read_byte(&arial_narrow[3]); //Количество симфолов в шрифте
	unsigned char fontWidth = pgm_read_byte(&arial_narrow[sign - amountOfElems]);  //Ширина
	// Находим координату в массиве. Т.к начинаеются с сотого и пока не совпадет с нашим суммируем
	unsigned char l=100;      // Начальное значение откуда начинаются символы
	...
	while(l != sign){
		widthSum += pgm_read_byte(&arial_narrow[widthStartAddres]);
		widthStartAddres++;
		l++;
	}
	widthSum *= 2;
	int charStartPos = 100+widthSum;
	if(column <= 64 && page < 6) // Проверяем установку столбца и строки
	{	
		for(uint8_t k = 0;k<fontHeight/8;k++){
			for(uint8_t i = 0; i < fontWidth; i++){
				scr_buffer[65*(page+k) + column + i] = pgm_read_byte(&arial_narrow[charStartPos+(fontWidth*k) + i]);
			}
		}
	}
	return fontWidth;
}
void print_string_arial(uint8_t column, uint8_t string, char *str) // 0..120 / 0..7 / Строка
{	
	unsigned char fontHeight = pgm_read_byte(&arial_narrow[1]); // Высота
	unsigned char fontWidth;
	while(*str)
	{
		fontWidth = print_arial(column, string, *str++);
		column+=fontWidth;
		if (column >= 64) {
			column-=64;
			string+=fontHeight/8;
...
int main(void)
{
	...
	while (1)
	{
		...
		if(invers==0){send_command(0xA7);invers = 1;}
		else{send_command(0xA6);invers = 0;}
