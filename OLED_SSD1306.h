...
void update_display(){
	send_command(0x21); //set columm address
	send_command(0);  //set low col address
	send_command(127);  //set high col address
	
	send_command(0x22); //set page address
	send_command(0);
	send_command(7);
	for(uint16_t i = 0; i < 960; i++) send_data(scr_buffer[i]);
}
void clear_screen()
{
	uint16_t k;
...
void print_5x8(uint8_t column, uint8_t string, uint8_t sign) // 0..63 / 0..5 / Символ
{
	...
	if(column <= 120 && string < 5) // Проверяем установку столбца и строки
	{
		if((sign >= 0x20) && (sign <= 0x7F))
		sign -= 32;// Смещение в таблице для символов ASCII[0x20-0x7F]
		else if (sign >= 0xC0)
